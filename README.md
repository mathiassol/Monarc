...
Presets: `msvc-debug`, `msvc-release`, `clang-debug`, `clang-release`, `clang-asan`
(AddressSanitizer) and `clang-ubsan` (UndefinedBehaviorSanitizer). They pin no absolute
...
All six run in CI on every push.
